...
def _load():
    ...
    df = pd.read_excel(_XLSX_PATH, sheet_name=_resolve_sheet_name(), engine='openpyxl', usecols=_keep_parametric_col)
...
def estimate_gcmr_fuel_lifetime(assembly_rings, core_rings, active_height,
    ...
    q_full = np.array([enrichment, assembly_rings, core_rings,
    ...
    # operating power, so dropping P (column index 4) from the distance
    # metric makes subcriticality consistent across P. Otherwise queries
    # identical in (NA, NC, H, E) but different in P could disagree on
    # the verdict.
    P_IDX = 4
...
def _load_curves():
    ...
    df = pd.read_excel(_XLSX_PATH, sheet_name=sheet, engine='openpyxl', usecols=_keep_parametric_col)
    df = df[['Assembly Rings', 'Core Rings', 'Active Height',
