...
_parser.add_argument('--packing-fraction', type=float, default=0.3)
...
def gcmr_calc(list_varied_params, calc_id, output_dir="NS_TESTS"):
    ...
    update_params({
        ...
        'Compact Fuel Radius': list_varied_params['Compact Fuel Radius'],  # cm # The radius of the area that is occupied by the TRISO particles (fuel compact/ fuel element)
        'Packing Fraction': list_varied_params['Packing Fraction'],
        
        # Coolant channel and booster dimensions
        'Coolant Channel Radius': 0.35,  # cm
        'Moderator Booster Radii': [list_varied_params['Moderator Envelope Radius'] - list_varied_params['Moderator Envelope Thickness'] - list_varied_params['Moderator Liner Thickness'], list_varied_params['Moderator Envelope Radius'] - list_varied_params['Moderator Envelope Thickness'], list_varied_params['Moderator Envelope Radius']], # cm
        'Lattice Pitch': 2.25,
        'Assembly Rings': 6,
        'Core Rings': 5, # took a ring off to meet shutdown criteria
    })
    ...
    params['Active Height'] = list_varied_params['Active Height']
    ...
    update_params({
       ...
       'Burnup Steps': [0.1, 0.5, 1.0, 5.0, 10.0, 25.0, 40.0, 60.0, 80.0, 100.0, 120.0, 140.0]  # MWd_per_Kg
        })
    ...
    # TODO: if k-eff[BOL]<0 - can we skip all this?
    # **************************************************************************************************************************
    ...
    update_params({
        ...
        'Vessel Thickness': 3,  # cm — ASME Sec III Div 1 thin-shell with 4 MPa He, R=60-100 cm, S=138 MPa SA-508 at 350°C, +3 mm corrosion
        'Vessel Lower Plenum Height': 30,  # cm — GA MHTGR / HTR-PM-class flow distributor
        ...
        'Gap Between Cooling Vessel And Intake Vessel': 5,  # cm — Hejzlar & Buongiorno 2007 NED RVACS minimum
    ...
    # https://www.nationalacademies.org/read/12844/chapter/6#69 cites a ~10%/yr helium loss rate,
    ...
    cost_estimate = detailed_bottom_up_cost_estimate('cost/Cost_Database.xlsx', f"{output_dir}/output_GCMR_A_{calc_id}.xlsx")
